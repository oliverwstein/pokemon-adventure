...
fn generate_move_data(f: &mut fs::File) -> Result<(), Box<dyn std::error::Error>> {
    ...
    writeln!(f, "pub fn get_compiled_move_data() -> std::collections::HashMap<crate::moves::Move, crate::move_data::MoveData> {{")?;
    ...
    for (move_variant, ron_content) in moves {
        ...
        writeln!(f, "    map.insert(crate::moves::Move::{}, ron::from_str(r####\"{}\"####).unwrap());", move_variant, ron_content)?;
    }
...
fn generate_species_data(f: &mut fs::File) -> Result<(), Box<dyn std::error::Error>> {
    ...
    writeln!(f, "pub fn get_compiled_species_data() -> [Option<crate::pokemon::PokemonSpecies>; 151] {{")?;
    writeln!(f, "    let mut data = [const {{ None }}; 151];")?;
    
    for (species_variant, ron_content) in species {
        writeln!(f, "    data[crate::species::Species::{}.pokedex_number() as usize - 1] = Some(ron::from_str(r####\"{}\"####).unwrap());", species_variant, ron_content)?;
    }
...
fn generate_prefab_teams_data(f: &mut fs::File) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (team_name, ron_content) in teams {
        writeln!(f, "    map.insert(\"{}\", r####\"{}\"####);", team_name, ron_content)?;
    }
