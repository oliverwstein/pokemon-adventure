#[cfg(test)]
mod tests {
    use crate::battle::state::{BattleState, TurnRng, BattleEvent, GameState};
    ...
    fn create_test_pokemon(species: Species, moves: Vec<Move>, hp: u16, attack: u16) -> PokemonInst {
        ...
        PokemonInst {
            ...
            ivs: [15; 6],
            evs: [0; 6],
            curr_stats: [hp, attack, 80, 80, 80, 80],
            moves: pokemon_moves,
            status: None,
        }
    }
    ...
    fn test_probabilistic_multi_hit_logic() {
        ...
        // --- ADDED LOGGING ---
        println!("Generated {} events in probabilistic multi-hit test:", events.len());
        for event in events {
            println!("  {:?}", event);
        }
        // ---------------------
        ...
        // ASSERTIONS
        let hit_events: Vec<_> = events.iter().filter(|e| matches!(e, BattleEvent::MoveHit { move_used: Move::FurySwipes, .. })).collect();
        let damage_events: Vec<_> = events.iter().filter(|e| matches!(e, BattleEvent::DamageDealt { .. })).collect();

        // We forced exactly 3 hits.
        assert_eq!(hit_events.len(), 3, "Fury Swipes should have hit exactly 3 times");
        // The defender's move also deals damage, so we expect 3 + 1 = 4 damage events.
        assert_eq!(damage_events.len(), 4, "Should be 4 total damage events in the turn");
        assert!(matches!(battle_state.game_state, GameState::WaitingForBothActions), "Game should be ready for the next turn");
    }
    ...
    fn test_multi_hit_stops_on_faint() {
        ...
        let mut battle_state = BattleState::new("multi_hit_faint_test".to_string(), player1, player2);
        ...
        // ASSERTIONS
        let fury_swipes_hits = events.iter().filter(|e| {
            matches!(e, BattleEvent::MoveHit { move_used: Move::FurySwipes, .. })
        }).count();
        ...
        let faint_events = events.iter().filter(|e| matches!(e, BattleEvent::PokemonFainted { .. })).count();
        assert_eq!(faint_events, 1, "The defender should have fainted");
        assert!(matches!(battle_state.game_state, GameState::WaitingForPlayer2Replacement), "Game should be waiting for replacement");

        // We can also assert that only one damage event occurred in total, since the
        // fainted defender's turn is skipped.
        let damage_events = events.iter().filter(|e| matches!(e, BattleEvent::DamageDealt { .. })).count();
        assert_eq!(damage_events, 1, "Only the first, fatal hit should have dealt damage.");
    }
}