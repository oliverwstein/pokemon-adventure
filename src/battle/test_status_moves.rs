...
mod tests {
    ...
    fn test_status_move_swords_dance() {
        ...
        execute_attack_hit(0, 1, Move::SwordsDance, 0, &mut action_stack, &mut bus, &mut rng, &mut battle_state);
        ...
        assert!(events.iter().any(|e| matches!(e, BattleEvent::MoveUsed { move_used: Move::SwordsDance, .. })));
        assert!(events.iter().any(|e| matches!(e, BattleEvent::MoveHit { move_used: Move::SwordsDance, .. })));
        assert!(events.iter().any(|e| matches!(e, BattleEvent::StatStageChanged { 
    ...
    fn test_status_move_harden() {
        ...
        execute_attack_hit(0, 1, Move::Harden, 0, &mut action_stack, &mut bus, &mut rng, &mut battle_state);
        ...
        // Should have MoveUsed, MoveHit, and StatStageChanged events
        assert!(events.iter().any(|e| matches!(e, BattleEvent::MoveUsed { move_used: Move::Harden, .. })));
        assert!(events.iter().any(|e| matches!(e, BattleEvent::MoveHit { move_used: Move::Harden, .. })));
        assert!(events.iter().any(|e| matches!(e, BattleEvent::StatStageChanged { 
    ...
    fn test_other_category_status_move_thunder_wave() {
        ...
        execute_attack_hit(0, 1, Move::ThunderWave, 0, &mut action_stack, &mut bus, &mut rng, &mut battle_state);
        
        // Check that defender is now paralyzed
        let defender_status = &battle_state.players[1].team[0].as_ref().unwrap().status;
        assert!(matches!(defender_status, Some(crate::pokemon::StatusCondition::Paralysis)));
        
        // Verify the events generated
        let events = bus.events();
        
        // Should have MoveUsed, MoveHit, and PokemonStatusApplied events
        assert!(events.iter().any(|e| matches!(e, BattleEvent::MoveUsed { move_used: Move::ThunderWave, .. })));
        assert!(events.iter().any(|e| matches!(e, BattleEvent::MoveHit { move_used: Move::ThunderWave, .. })));
        assert!(events.iter().any(|e| matches!(e, BattleEvent::PokemonStatusApplied { 
