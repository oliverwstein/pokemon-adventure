...
mod tests {
    ...
    fn test_recover_heals_damaged_pokemon() {
        ...
        assert!(
            heal_event_found,
            "A PokemonHealed event should have been emitted for Chansey"
        );
        ...
        assert!(
            damage_event_found,
            "Chansey should have taken damage from Tackle after healing"
        );
    }
    ...
    fn test_recover_does_not_heal_at_full_hp() {
        ...
        assert!(
            !heal_event_found,
            "A PokemonHealed event should NOT be emitted when Recover is used at full HP"
        );
        ...
        assert!(
            recover_used_event,
            "Recover should still be registered as used"
        );
    }
    ...
    fn test_recover_does_not_heal_fainted_pokemon() {
        ...
        // Assert
        event_bus
            .print_debug_with_message("Events for test_recover_does_not_heal_fainted_pokemon:");

        // The action for the fainted Pokémon should fail.
        let action_failed_event = event_bus
            .events()
            .iter()
            .any(|e| matches!(e, BattleEvent::ActionFailed { .. }));
        assert!(
            action_failed_event,
            "The fainted Pokémon's action should have failed"
        );
