...
mod tests {
    ...
    fn test_mist_blocks_stat_reduction(#[case] desc: &str, #[case] debuff_move: Move) {
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(Species::Alakazam, 10).with_moves(vec![debuff_move]).build();
        ...
        // Assert
        event_bus.print_debug_with_message(&format!("Events for test_mist_blocks_stat_reduction [{}]:", desc));

        assert_eq!(battle_state.players[1].stat_stages, initial_stat_stages, "Mist should prevent all stat reductions");
        let blocked_event_found = event_bus.events().iter().any(|e| matches!(e, BattleEvent::StatChangeBlocked { .. }));
        let stage_changed_event_found = event_bus.events().iter().any(|e| matches!(e, BattleEvent::StatStageChanged { target: Species::Machamp, .. }));

        assert!(blocked_event_found, "A StatChangeBlocked event should have been emitted");
        assert!(!stage_changed_event_found, "No StatStageChanged event should be emitted for the target when blocked");
    }

    #[test]
    fn test_mist_allows_self_targeting_stat_increases() {
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(Species::Alakazam, 10).with_moves(vec![Move::SwordsDance]).build();
        let p2_pokemon = TestPokemonBuilder::new(Species::Machamp, 10).with_moves(vec![Move::Splash]).build();
        ...
        // Assert
        event_bus.print_debug_with_message("Events for test_mist_allows_self_targeting_stat_increases:");

        assert_eq!(battle_state.players[0].get_stat_stage(StatType::Attack), 2, "Attack should be raised by Swords Dance");
        let blocked_event_found = event_bus.events().iter().any(|e| matches!(e, BattleEvent::StatChangeBlocked { .. }));
        assert!(!blocked_event_found, "Mist should not block self-targeting stat increases");
    }

    #[test]
    fn test_mist_does_not_block_damaging_moves() {
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(Species::Alakazam, 10).with_moves(vec![Move::Tackle]).build();
        let p2_pokemon = TestPokemonBuilder::new(Species::Machamp, 10).with_moves(vec![Move::Splash]).build();
        ...
        let blocked_event_found = event_bus.events().iter().any(|e| matches!(e, BattleEvent::StatChangeBlocked { .. }));
        let damage_event_found = event_bus.events().iter().any(|e| matches!(e, BattleEvent::DamageDealt { target: Species::Machamp, .. }));

        assert!(!blocked_event_found, "Mist should not block moves that only deal damage");
        assert!(damage_event_found, "A damage-only move should still hit a target protected by Mist");
    }

    #[test]
    fn test_mist_expires_normally() {
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(Species::Alakazam, 10).with_moves(vec![Move::Growl]).build();
        let p2_pokemon = TestPokemonBuilder::new(Species::Machamp, 10).with_moves(vec![Move::Splash]).build();
        ...
        assert!(!battle_state.players[1].has_team_condition(&TeamCondition::Mist), "Mist should have expired");
        assert!(bus1.events().iter().any(|e| matches!(e, BattleEvent::TeamConditionExpired { condition: TeamCondition::Mist, .. })));
        ...
        assert_eq!(battle_state.players[1].get_stat_stage(StatType::Attack), -1, "Growl should now lower Attack");
        let blocked_event_found = bus2.events().iter().any(|e| matches!(e, BattleEvent::StatChangeBlocked { .. }));
        assert!(!blocked_event_found, "Stat reduction should not be blocked after Mist expires");
    }
}