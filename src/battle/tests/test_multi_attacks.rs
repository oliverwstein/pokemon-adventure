...
mod tests {
    ...
    fn test_probabilistic_multi_hit_logic(
        ...
        // Assert
        event_bus.print_debug_with_message(&format!("Events for test_probabilistic_multi_hit_logic [{}]:", desc));

        let fury_swipes_hits = event_bus.events().iter()
            .filter(|e| matches!(e, BattleEvent::MoveHit { move_used: Move::FurySwipes, .. }))
            .count();

        // Total damage events = fury swipes hits + opponent's tackle
        let total_damage_events = event_bus.events().iter()
            .filter(|e| matches!(e, BattleEvent::DamageDealt { .. }))
            .count();

        assert_eq!(fury_swipes_hits, expected_hits, "Fury Swipes should have hit the expected number of times");
        assert_eq!(total_damage_events, expected_hits + 1, "Should be one damage event per hit + one for the opponent's move");
    ...
    fn test_multi_hit_stops_on_faint() {
        ...
        let attacker = TestPokemonBuilder::new(Species::Meowth, 25).with_moves(vec![Move::FurySwipes]).build();
        let defender = TestPokemonBuilder::new(Species::Pidgey, 5).with_moves(vec![Move::Tackle]).with_hp(1).build();
        ...
        let faint_events = event_bus.events().iter()
            .filter(|e| matches!(e, BattleEvent::PokemonFainted { player_index: 1, .. }))
            .count();

        assert_eq!(fury_swipes_hits, 1, "The multi-hit sequence should stop after the first fatal hit");
