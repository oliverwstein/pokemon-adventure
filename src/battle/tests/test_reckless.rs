...
mod tests {
    ...
    #[rstest]
    #[case(
        "HighJumpKick misses and causes 50% recoil",
        Move::HighJumpKick,
        95,
        50,
        false
    )] // HJK accuracy is 80, so 95 is a miss
    #[case(
        "HighJumpKick hits and causes no recoil",
        Move::HighJumpKick,
        50,
        0,
        false
    ...
    #[case(
        "HighJumpKick misses and causes fainting",
        Move::HighJumpKick,
        95,
        50,
        true
    )] // Test recoil fainting
    fn test_reckless_move_outcomes(
        ...
        // Arrange
        let mut p1_builder =
            TestPokemonBuilder::new(Species::Hitmonlee, 50).with_moves(vec![reckless_move]);
        ...
        event_bus.print_debug_with_message(&format!(
            "Events for test_reckless_move_outcomes [{}]:",
            desc
        ));

        let move_missed = event_bus.events().iter().any(|e| {
            matches!(
                e,
        ...
        let recoil_damage_event = event_bus.events().iter().find(|e| {
            matches!(
                e,
        ...
        let fainted_event = event_bus.events().iter().any(|e| {
            matches!(
                e,
        ...
        if expected_recoil_percent > 0 {
            ...
            assert!(
                recoil_damage_event.is_some(),
                "Recoil damage should have been dealt to the attacker"
            );

            if should_faint {
                assert!(
                    fainted_event,
                    "Attacker should have fainted from recoil damage"
                );
            } else {
                assert!(
                    !fainted_event,
                    "Attacker should not have fainted from recoil damage"
                );
            }
        } else {
    ...
    fn test_non_reckless_move_has_no_recoil_on_miss() {
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(Species::Hitmonlee, 10)
            .with_moves(vec![Move::Tackle])
            .build();
        ...
        // Assert
        event_bus
            .print_debug_with_message("Events for test_non_reckless_move_has_no_recoil_on_miss:");
        ...
        let recoil_damage_event = event_bus.events().iter().any(|e| {
            matches!(
                e,
                BattleEvent::DamageDealt {
                    target: Species::Hitmonlee,
        ...
        assert!(
            !recoil_damage_event,
            "Non-reckless moves should not cause recoil damage on miss"
        );
    }
}
