...
mod tests {
    ...
    fn test_reckless_effect_on_miss() {
        ...
        let player1 = BattlePlayer::new(
            "player1".to_string(),
            "Player 1".to_string(),
            vec![create_test_pokemon(Species::Hitmonlee, vec![Move::HighJumpKick])], // HighJumpKick has Reckless(50)
        );
        ...
        // Should have missed
        let miss_events: Vec<_> = event_bus.events().iter()
            .filter(|event| matches!(event, BattleEvent::MoveMissed { attacker: Species::Hitmonlee, move_used: Move::HighJumpKick, .. }))
            .collect();
        ...
        let recoil_damage_events: Vec<_> = event_bus.events().iter()
            ...
            .collect();
        assert!(!recoil_damage_events.is_empty(), "Should have taken {} recoil damage from missed High Jump Kick", expected_recoil);

        // Verify final HP accounts for recoil damage (and possibly damage from Tackle)
        assert!(final_hp < initial_hp, "Pokemon should have taken damage from recoil (and possibly Tackle)");
    }
    ...
    fn test_reckless_effect_on_hit_no_recoil() {
        ...
        let player1 = BattlePlayer::new(
            "player1".to_string(),
            "Player 1".to_string(),
            vec![create_test_pokemon(Species::Hitmonlee, vec![Move::HighJumpKick])],
        );
        ...
        let miss_events: Vec<_> = event_bus.events().iter()
            .filter(|event| matches!(event, BattleEvent::MoveMissed { attacker: Species::Hitmonlee, .. }))
            .collect();
        assert!(miss_events.is_empty(), "High Jump Kick should have hit, not missed");

        // Should have hit instead
        let hit_events: Vec<_> = event_bus.events().iter()
            .filter(|event| matches!(event, BattleEvent::MoveHit { attacker: Species::Hitmonlee, move_used: Move::HighJumpKick, .. }))
            .collect();
        ...
        let recoil_damage_events: Vec<_> = event_bus.events().iter()
            .filter(|event| matches!(event, BattleEvent::DamageDealt { target: Species::Hitmonlee, .. }))
            .collect();
        ...
        if !recoil_damage_events.is_empty() {
            for event in &recoil_damage_events {
                if let BattleEvent::DamageDealt { damage, .. } = event {
                    ...
                    assert!(*damage != recoil_amount, "Should not have recoil damage when move hits");
                }
            }
        }
    }
    ...
    fn test_reckless_different_percentages() {
        ...
        let player1 = BattlePlayer::new(
            "player1".to_string(),
            "Player 1".to_string(),
            vec![create_test_pokemon(Species::Hitmonlee, vec![Move::JumpKick])], // JumpKick has Reckless(20)
        );
        ...
        // Should have missed
        let miss_events: Vec<_> = event_bus.events().iter()
            .filter(|event| matches!(event, BattleEvent::MoveMissed { attacker: Species::Hitmonlee, move_used: Move::JumpKick, .. }))
            .collect();
        ...
        let recoil_damage_events: Vec<_> = event_bus.events().iter()
            .filter(|event| matches!(event, BattleEvent::DamageDealt { target: Species::Hitmonlee, damage, .. } if *damage == expected_recoil))
            .collect();
        assert!(!recoil_damage_events.is_empty(), "Should have taken {} recoil damage from missed Jump Kick", expected_recoil);
    }
    ...
    fn test_reckless_can_cause_fainting() {
        ...
        assert!(low_hp > 0 && low_hp <= recoil_damage, "Pokemon should have low HP but not be fainted yet");
        ...
        assert_eq!(final_hp, 0, "Pokemon should have fainted from recoil damage");

        // Should have PokemonFainted event
        let faint_events: Vec<_> = event_bus.events().iter()
            .filter(|event| matches!(event, BattleEvent::PokemonFainted { pokemon: Species::Hitmonlee, player_index: 0 }))
            .collect();
        assert!(!faint_events.is_empty(), "Should have PokemonFainted event from recoil damage");
    }
    ...
    fn test_non_reckless_move_no_recoil_on_miss() {
        ...
        let self_damage_events: Vec<_> = event_bus.events().iter()
            .filter(|event| matches!(event, BattleEvent::DamageDealt { target: Species::Hitmonlee, .. }))
            .collect();
        ...
        for event in &self_damage_events {
            if let BattleEvent::DamageDealt { damage, .. } = event {
                ...
                let max_hp = initial_hp;
                assert!(*damage < max_hp / 4, "Damage should be from opponent's attack, not recoil");
            }
        }
    }
}