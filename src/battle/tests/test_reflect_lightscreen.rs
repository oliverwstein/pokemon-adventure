...
mod tests {
    ...
    fn test_reflect_reduces_physical_damage() {
        ...
        let mut battle_state_baseline = BattleState::new("test_battle_baseline".to_string(), player1.clone(), player2.clone());
        ...
        let test_rng_baseline = TurnRng::new_for_test(vec![50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50]);
        let event_bus_baseline = resolve_turn(&mut battle_state_baseline, test_rng_baseline);

        // Get baseline damage
        let baseline_damage = event_bus_baseline.events().iter()
            .find_map(|event| match event {
                BattleEvent::DamageDealt { target: Species::Alakazam, damage, .. } => Some(*damage),
                _ => None,
            })
        ...
        let mut battle_state_reflect = BattleState::new("test_battle_reflect".to_string(), player1, player2_reflect);
        ...
        let test_rng_reflect = TurnRng::new_for_test(vec![50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50]);
        let event_bus_reflect = resolve_turn(&mut battle_state_reflect, test_rng_reflect);

        // Get Reflect damage
        let reflect_damage = event_bus_reflect.events().iter()
            .find_map(|event| match event {
                BattleEvent::DamageDealt { target: Species::Alakazam, damage, .. } => Some(*damage),
                _ => None,
            })
        ...
        println!("  Damage reduction: {}%", (100 * (baseline_damage - reflect_damage)) / baseline_damage);

        // Reflect should reduce damage by approximately 50%
        assert!(reflect_damage < baseline_damage, "Reflect should reduce damage");
        ...
        assert!(damage_difference <= 2, "Reflect damage should be approximately half of baseline damage. Expected: ~{}, Actual: {}", expected_reflect_damage, reflect_damage);
    }
    ...
    fn test_light_screen_reduces_special_damage() {
        ...
        let player1 = BattlePlayer::new(
            "player1".to_string(),
            "Player 1".to_string(),
            vec![create_test_pokemon(Species::Alakazam, vec![Move::Confusion])], // Special attacker
        );
        ...
        let baseline_damage = event_bus_baseline.events().iter()
            .find_map(|event| match event {
                BattleEvent::DamageDealt { target: Species::Machamp, damage, .. } => Some(*damage),
                _ => None,
            })
            .expect("Should have damage event in baseline test");
        ...
        let mut battle_state_lightscreen = BattleState::new("test_battle_lightscreen".to_string(), player1, player2_lightscreen);
        ...
        let test_rng_lightscreen = TurnRng::new_for_test(vec![50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50]);
        let event_bus_lightscreen = resolve_turn(&mut battle_state_lightscreen, test_rng_lightscreen);

        // Get Light Screen damage
        let lightscreen_damage = event_bus_lightscreen.events().iter()
            .find_map(|event| match event {
                BattleEvent::DamageDealt { target: Species::Machamp, damage, .. } => Some(*damage),
                _ => None,
            })
        ...
        println!("  Damage reduction: {}%", (100 * (baseline_damage - lightscreen_damage)) / baseline_damage);

        // Light Screen should reduce damage by approximately 50%
        assert!(lightscreen_damage < baseline_damage, "Light Screen should reduce damage");
        ...
        assert!(damage_difference <= 2, "Light Screen damage should be approximately half of baseline damage. Expected: ~{}, Actual: {}", expected_lightscreen_damage, lightscreen_damage);
    }
    ...
    fn test_reflect_does_not_reduce_special_damage() {
        ...
        let player1 = BattlePlayer::new(
            "player1".to_string(),
            "Player 1".to_string(),
            vec![create_test_pokemon(Species::Alakazam, vec![Move::Confusion])], // Special move
        );
        ...
        let mut battle_state_no_reflect = BattleState::new("test_battle".to_string(), player1.clone(), player2.clone());
        ...
        let test_rng_no_reflect = TurnRng::new_for_test(vec![50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50]);
        let event_bus_no_reflect = resolve_turn(&mut battle_state_no_reflect, test_rng_no_reflect);

        let damage_without_reflect = event_bus_no_reflect.events().iter()
            .find_map(|event| match event {
                BattleEvent::DamageDealt { target: Species::Machamp, damage, .. } => Some(*damage),
                _ => None,
            })
        ...
        let mut battle_state_reflect = BattleState::new("test_battle".to_string(), player1, player2_reflect);
        ...
        let damage_with_reflect = event_bus_reflect.events().iter()
            .find_map(|event| match event {
                BattleEvent::DamageDealt { target: Species::Machamp, damage, .. } => Some(*damage),
                _ => None,
            })
        ...
        assert_eq!(damage_without_reflect, damage_with_reflect, "Reflect should not affect special damage");
    }
    ...
    fn test_light_screen_does_not_reduce_physical_damage() {
        ...
        let mut battle_state_no_lightscreen = BattleState::new("test_battle".to_string(), player1.clone(), player2.clone());
        ...
        let test_rng_no_lightscreen = TurnRng::new_for_test(vec![50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50]);
        let event_bus_no_lightscreen = resolve_turn(&mut battle_state_no_lightscreen, test_rng_no_lightscreen);

        let damage_without_lightscreen = event_bus_no_lightscreen.events().iter()
            .find_map(|event| match event {
                BattleEvent::DamageDealt { target: Species::Alakazam, damage, .. } => Some(*damage),
                _ => None,
            })
        ...
        let mut battle_state_lightscreen = BattleState::new("test_battle".to_string(), player1, player2_lightscreen);
        ...
        let damage_with_lightscreen = event_bus_lightscreen.events().iter()
            .find_map(|event| match event {
                BattleEvent::DamageDealt { target: Species::Alakazam, damage, .. } => Some(*damage),
                _ => None,
            })
        ...
        println!("  Damage without Light Screen: {}", damage_without_lightscreen);
        ...
        assert_eq!(damage_without_lightscreen, damage_with_lightscreen, "Light Screen should not affect physical damage");
    }
}