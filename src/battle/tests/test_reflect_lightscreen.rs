...
mod tests {
    ...
    fn get_damage_from_turn(
        ...
        let mut battle_state = BattleState::new("test".to_string(), attacker_player, defender_player);
        ...
        let damage = event_bus.events().iter()
    ...
    #[rstest]
    #[case("Reflect reduces Physical damage", Move::Tackle, Species::Alakazam, Some(TeamCondition::Reflect), true)]
    #[case("Light Screen reduces Special damage", Move::Confusion, Species::Machamp, Some(TeamCondition::LightScreen), true)]
    #[case("Reflect does NOT reduce Special damage", Move::Confusion, Species::Machamp, Some(TeamCondition::Reflect), false)]
    #[case("Light Screen does NOT reduce Physical damage", Move::Tackle, Species::Alakazam, Some(TeamCondition::LightScreen), false)]
    fn test_screen_effects_on_damage(
        ...
        // Arrange
        let attacker = TestPokemonBuilder::new(Species::Machamp, 50).with_moves(vec![attacking_move]).build();
        let defender = TestPokemonBuilder::new(defender_species, 50).with_moves(vec![Move::Splash]).build();

        // --- Baseline Run (No Screen) ---
        let baseline_defender_player = create_test_player("p2", "Player 2", vec![defender.clone()]);
        let (baseline_damage, baseline_bus) = get_damage_from_turn(attacker.clone(), baseline_defender_player);
        ...
        let (modified_damage, modified_bus) = get_damage_from_turn(attacker, modified_defender_player);

        // Assert
        println!("\n--- Test Case: {} ---", desc);
        baseline_bus.print_debug_with_message("Baseline Events:");
        modified_bus.print_debug_with_message("Modified Events:");
        println!("Baseline Damage: {}, Modified Damage: {}", baseline_damage, modified_damage);

        if expect_reduction {
            assert!(modified_damage > 0, "Modified damage should be greater than 0");
            assert!(modified_damage < baseline_damage, "Damage should have been reduced");
            // Check if damage is roughly halved, allowing for rounding
            let expected_damage = baseline_damage / 2;
            assert!((modified_damage as i16 - expected_damage as i16).abs() <= 2, "Damage reduction should be approx. 50%");
        } else {
            assert_eq!(modified_damage, baseline_damage, "Damage should be identical as the screen has no effect");
        }
    }
}