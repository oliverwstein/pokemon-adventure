...
mod tests {
    ...
    #[rstest]
    #[case("Charging (SolarBeam)", Move::SolarBeam, PokemonConditionType::Charging, Species::Venusaur, Species::Charizard)]
    #[case("InAir (Fly)", Move::Fly, PokemonConditionType::InAir, Species::Pidgeot, Species::Rattata)]
    #[case("Underground (Dig)", Move::Dig, PokemonConditionType::Underground, Species::Sandslash, Species::Geodude)]
    fn test_two_turn_moves(
        ...
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(attacker_species, 25).with_moves(vec![two_turn_move]).build();
        let p2_pokemon = TestPokemonBuilder::new(defender_species, 25).with_moves(vec![Move::Tackle]).build();
        ...
        assert!(battle_state.players[0].has_condition_type(expected_condition), "Player should have the correct two-turn condition");
        assert_eq!(battle_state.players[0].last_move, Some(two_turn_move));
        assert!(battle_state.action_queue[0].is_some(), "Action queue should be pre-filled for Turn 2");

        // --- TURN 2: Execute the move ---
        let npc_actions = collect_npc_actions(&battle_state);
        for (i, action) in npc_actions { battle_state.action_queue[i] = Some(action); }
        ...
        assert!(!battle_state.players[0].has_condition_type(expected_condition), "Two-turn condition should be cleared after execution");
        ...
        let damage_dealt = bus2.events().iter().any(|e| matches!(e, BattleEvent::DamageDealt { target, .. } if *target == defender_species));
    ...
    fn test_rampage_forces_action_and_ends_with_confusion() {
        ...
        collect_npc_actions(&battle_state).into_iter().for_each(|(i, a)| battle_state.action_queue[i] = Some(a));
        let bus2 = resolve_turn(&mut battle_state, predictable_rng());

        // Assert - Turn 2
        bus2.print_debug_with_message("Events for Rampage [Turn 2 - Final Turn]:");
        assert!(bus2.events().iter().any(|e| matches!(e, BattleEvent::MoveUsed { player_index: 0, move_used: Move::Thrash, .. })));
        
        // After this turn, the rampage ends and confusion should be applied.
        assert!(!battle_state.players[0].has_condition_type(PokemonConditionType::Rampaging), "Rampaging should be cleared");
        assert!(battle_state.players[0].has_condition_type(PokemonConditionType::Confused), "Should become confused after rampage");
        assert!(bus2.events().iter().any(|e| matches!(e, BattleEvent::StatusApplied { status: PokemonCondition::Confused { .. }, .. })));
    }
    ...
    fn test_mirror_move_outcomes(#[case] desc: &str, #[case] opponent_last_move: Option<Move>, #[case] should_succeed: bool) {
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(Species::Pidgeot, 25).with_moves(vec![Move::MirrorMove]).build();
        let p2_pokemon = TestPokemonBuilder::new(Species::Pikachu, 25).with_moves(vec![Move::Tackle]).build();
        ...
        // Assert
        event_bus.print_debug_with_message(&format!("Events for test_mirror_move_outcomes [{}]:", desc));
        
        let move_failed = event_bus.events().iter().any(|e| matches!(e, BattleEvent::ActionFailed { .. }));
        ...
        if should_succeed {
            ...
            assert!(mirrored_move_used, "The mirrored move should have been used");
        } else {
    ...
    fn test_explosion_faints_user_and_deals_damage() {
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(Species::Electrode, 50).with_moves(vec![Move::Explosion]).build();
        let p2_pokemon = TestPokemonBuilder::new(Species::Snorlax, 50).with_moves(vec![Move::Rest]).build();
        let mut battle_state = create_test_battle(p1_pokemon, p2_pokemon);
        ...
        // Assert
        event_bus.print_debug_with_message("Events for test_explosion_faints_user_and_deals_damage:");
        assert!(battle_state.players[0].active_pokemon().unwrap().is_fainted(), "User should have fainted from Explosion");
        let faint_event = event_bus.events().iter().any(|e| matches!(e, BattleEvent::PokemonFainted { player_index: 0, .. }));
        let damage_event = event_bus.events().iter().any(|e| matches!(e, BattleEvent::DamageDealt { target: Species::Snorlax, .. }));

        assert!(faint_event, "A fainted event should be present for the user");
        assert!(damage_event, "A damage event should be present for the target");
    }
    ...
    fn test_transformed_and_converted_conditions_unit() {
        ...
        assert_eq!(transformed_types, vec![PokemonType::Fire, PokemonType::Flying], "Should have Charizard's types");

        // Test Converted (should override Transformed)
        player.add_condition(PokemonCondition::Converted { pokemon_type: PokemonType::Electric });
        let converted_types = player.active_pokemon().unwrap().get_current_types(&player);
        assert_eq!(converted_types, vec![PokemonType::Electric], "Converted should override Transform");
    }
    ...
    #[case("status effect", Move::ThunderWave, PokemonConditionType::Substitute, false)] // Substitute should block status
    #[case("stat decrease", Move::SandAttack, PokemonConditionType::Substitute, false)] // Substitute should block stat drops
    #[case("active condition", Move::ConfuseRay, PokemonConditionType::Substitute, false)] // Substitute should block confusion
    fn test_substitute_blocks_effects(
        ...
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(Species::Pikachu, 25).with_moves(vec![incoming_move]).build();
        let p2_pokemon = TestPokemonBuilder::new(Species::Alakazam, 25).with_moves(vec![Move::Tackle]).build();
        ...
        // Assert
        event_bus.print_debug_with_message(&format!("Events for test_substitute_blocks_effects [{}]:", desc));
