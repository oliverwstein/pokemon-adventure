...
mod tests {
    ...
    fn test_switch_event_shows_correct_pokemon() {
        ...
        let mut battle_state = crate::battle::state::BattleState::new(
            "test".to_string(), 
            player1, 
            player2
        );
        ...
        let switch_events: Vec<_> = bus.events().iter()
            .filter(|e| matches!(e, BattleEvent::PokemonSwitched { .. }))
            .collect();
        
        assert_eq!(switch_events.len(), 1, "Should have exactly one switch event");
        ...
        if let BattleEvent::PokemonSwitched { 
            ...
            assert_ne!(*old_pokemon, *new_pokemon, "Old and new Pokemon should be different");
        } else {
    ...
    fn test_switch_between_same_species_different_individuals() {
        ...
        // Verify initial state
        assert_eq!(
            battle_state.players[0].active_pokemon().unwrap().current_hp(),
            50
        );
