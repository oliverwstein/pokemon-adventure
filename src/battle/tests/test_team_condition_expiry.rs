...
mod tests {
    ...
    fn test_reflect_expires_after_turns() {
        ...
        assert_eq!(
            player2.get_team_condition_turns(&TeamCondition::Reflect),
            Some(3)
        );
        ...
        assert_eq!(
            battle_state.players[1].get_team_condition_turns(&TeamCondition::Reflect),
            Some(2)
        );
    ...
    fn test_light_screen_expires_after_turns() {
        ...
        assert_eq!(
            player2.get_team_condition_turns(&TeamCondition::LightScreen),
            Some(2)
        );
        ...
        assert_eq!(
            battle_state.players[1].get_team_condition_turns(&TeamCondition::LightScreen),
            Some(1)
        );
    ...
    fn test_team_condition_effectiveness_changes_on_expiry() {
        ...
        let turn1_damage = event_bus1.events().iter().find_map(|event| match event {
            crate::battle::state::BattleEvent::DamageDealt {
        ...
        let turn2_damage = event_bus2.events().iter().find_map(|event| match event {
            crate::battle::state::BattleEvent::DamageDealt {
                target: Species::Alakazam,
                damage,
                ..
        ...
        if let (Some(damage1), Some(damage2)) = (turn1_damage, turn2_damage) {
            ...
            assert!(
                damage2 > damage1,
                "Damage without Reflect ({}) should be greater than damage with Reflect ({})",
                damage2,
                damage1
            );
            ...
            assert!(
                damage_difference <= 2,
                "Turn 2 damage should be roughly double turn 1 damage. Expected: ~{}, Actual: {}",
                expected_turn2_damage,
                damage2
            );
        } else {
    ...
    fn test_team_condition_ticking_in_isolation() {
        ...
        assert_eq!(
            player.get_team_condition_turns(&TeamCondition::LightScreen),
            Some(1)
        );
