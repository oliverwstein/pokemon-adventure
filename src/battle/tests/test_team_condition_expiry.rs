...
mod tests {
    ...
    fn test_team_condition_ticking_unit() {
        ...
        assert_eq!(
            player.get_team_condition_turns(&TeamCondition::Reflect),
            Some(2)
        );
    ...
    fn test_team_condition_expires_after_duration(
        ...
        // Arrange
        let p1_pokemon = TestPokemonBuilder::new(Species::Machamp, 10)
            .with_moves(vec![Move::Tackle])
            .build();
        let p2_pokemon = TestPokemonBuilder::new(Species::Alakazam, 10)
            .with_moves(vec![Move::Splash])
            .build();
        ...
        for i in 1..=duration {
            ...
            if i < duration {
                assert!(
                    battle_state.players[1].has_team_condition(&condition),
                    "Condition should be active before final turn"
                );
            }
        }
        ...
        assert!(
            !battle_state.players[1].has_team_condition(&condition),
            "Condition should have expired after {} turns",
            duration
        );
    }
    ...
    fn test_multiple_team_conditions_expire_independently() {
        ...
        assert!(
            battle_state.players[1].has_team_condition(&TeamCondition::Reflect),
            "Reflect should still be active"
        );
        assert!(
            !battle_state.players[1].has_team_condition(&TeamCondition::LightScreen),
            "Light Screen should have expired"
        );
        assert!(bus1.events().iter().any(|e| matches!(
            e,
            BattleEvent::TeamConditionExpired {
                condition: TeamCondition::LightScreen,
                ..
            }
        )));
        ...
        assert!(bus2.events().iter().any(|e| matches!(
            e,
            BattleEvent::TeamConditionExpired {
                condition: TeamCondition::Reflect,
                ..
            }
        )));
    }
    ...
    fn test_effectiveness_changes_on_expiry() {
        ...
        let attacker = TestPokemonBuilder::new(Species::Machamp, 50)
            .with_moves(vec![Move::Tackle])
            .build();
        let defender = TestPokemonBuilder::new(Species::Alakazam, 50)
            .with_moves(vec![Move::Splash])
            .build();
        ...
        assert!(
            !battle_state.players[1].has_team_condition(&TeamCondition::Reflect),
            "Reflect should expire after turn 1"
        );
        ...
        bus2.print_debug_with_message(
            "Events for effectiveness change [Turn 2 - Without Reflect]:",
        );
        let damage2 = bus2
            .events()
            .iter()
        ...
        assert!(
            damage2 > damage1,
            "Damage should be higher after Reflect expires"
        );
    }
}
