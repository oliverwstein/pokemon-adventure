    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            MoveEffect::Flinch(chance) => {
                write!(f, "Has a {}% chance to make the target flinch.", chance)
            }
            ...
            MoveEffect::Freeze(chance) => {
                write!(f, "Has a {}% chance to freeze the target.", chance)
            }
            MoveEffect::Paralyze(100) => write!(f, "Paralyzes the target."),
            MoveEffect::Paralyze(chance) => {
                write!(f, "Has a {}% chance to paralyze the target.", chance)
            }
            MoveEffect::Poison(100) => write!(f, "Poisons the target."),
            MoveEffect::Poison(chance) => {
                write!(f, "Has a {}% chance to poison the target.", chance)
            }
            MoveEffect::Sedate(100) => write!(f, "Puts the target to sleep."),
            MoveEffect::Sedate(chance) => {
                write!(f, "Has a {}% chance to put the target to sleep.", chance)
            }
            MoveEffect::Confuse(100) => write!(f, "Confuses the target."),
            MoveEffect::Confuse(chance) => {
                write!(f, "Has a {}% chance to confuse the target.", chance)
            }
            MoveEffect::Trap(100) => {
                write!(f, "Traps the target, preventing escape for several turns.")
            }
            MoveEffect::Trap(chance) => write!(f, "Has a {}% chance to trap the target.", chance),
            MoveEffect::Seed(100) => {
                write!(f, "Plants a seed on the target, draining HP each turn.")
            }
            MoveEffect::Seed(chance) => {
                write!(f, "Has a {}% chance to plant a seed on the target.", chance)
            }
            MoveEffect::Disable(100) => write!(f, "Disables the last move the target used."),
            MoveEffect::Disable(chance) => write!(
                f,
                "Has a {}% chance to disable the target's last move.",
                chance
            ),
            ...
            MoveEffect::StatChange(target, stat, stages, chance) => {
                if *chance == 100 {
                    ...
                    write!(
                        f,
                        "Has a {}% chance to {} the {} {} by {} stage(s).",
                        chance,
                        action,
                        target,
                        stat,
            ...
            MoveEffect::RaiseAllStats(chance) => write!(
                f,
                "Has a {}% chance to raise all of the user's primary stats.",
                chance
            ),
            MoveEffect::Haze(_) => write!(
                f,
                "Eliminates all stat changes for all Pokémon on the field."
            ),

            // --- DAMAGE MODIFIERS AND HP EFFECTS ---
            MoveEffect::Recoil(percent) => write!(
                f,
                "User receives recoil damage equal to {}% of the damage dealt.",
                percent
            ),
            MoveEffect::Drain(percent) => match percent {
                ...
                _ => write!(
                    f,
                    "User recovers HP equal to {}% of the damage dealt.",
                    percent
                ),
            },
            ...
            MoveEffect::Heal(percent) => match percent {
                ...
                _ => write!(
                    f,
                    "Restores the user's HP by {}% of its maximum HP.",
                    percent
                ),
            },
            ...
            MoveEffect::MultiHit(guaranteed_hits, continuation_chance) => {
                match (*guaranteed_hits, *continuation_chance) {
                    // g > 1, c > 0
                    (g, c) if g > 1 && c > 0 => write!(
                        f,
                        "A series of {} attacks with a {}% chance of making more.",
                        g, c
                    ),
                    // g > 1, c == 0
            ...
            MoveEffect::ChargeUp => {
                write!(f, "Charges on the first turn, then attacks on the second.")
            }
            MoveEffect::InAir => write!(
                f,
                "User flies up on the first turn, then attacks on the second."
            ),
            MoveEffect::Underground => write!(
                f,
                "User goes underground on the first turn, then attacks on the second."
            ),
            MoveEffect::Exhaust(100) => write!(f, "User must recharge on the next turn."),
            MoveEffect::Exhaust(chance) => write!(
                f,
                "Has a {}% chance to exhaust the user for a turn after use",
                chance
            ),
            MoveEffect::Rampage => write!(
                f,
                "User locks in to one attack for 2-3 turns, then becomes confused."
            ),
            ...
            MoveEffect::Conversion => write!(
                f,
                "Changes the user's type to match the type of its first move."
            ),
            MoveEffect::Counter => write!(
                f,
                "Braces for impact, then returns twice the damage received this turn."
            ),
            ...
            MoveEffect::Substitute => write!(
                f,
                "Creates a substitute with 25% of the user's max HP that blocks secondary effects."
            ),
            MoveEffect::Rest(turns) => write!(
                f,
                "User sleeps for {} turns, fully restoring HP and status.",
                turns
            ),
            MoveEffect::Bide(turns) => write!(
                f,
                "User absorbs damage for {} turns, then retaliates with double the power.",
                turns
            ),
            MoveEffect::Rage(100) => write!(
                f,
                "Induces a trance that raises attack when hit until a different move is used."
            ),
            MoveEffect::Rage(chance) => write!(
                f,
                "Has a {}% chance to induce a trance that raises attack when hit until a different move is used.",
                chance
            ),
            MoveEffect::Teleport(100) => {
                write!(f, "Moves in a flash to dodge enemy blows this turn.")
            }
            MoveEffect::Teleport(chance) => write!(
                f,
                "Has a {}% chance to dodge enemy blows this turn.",
                chance
            ),
            ...
            // --- UTILITY ---
            MoveEffect::CureStatus(target, status) => {
                write!(f, "Cures the {} of {:?}.", target, status)
            }
...
impl fmt::Display for MoveData {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            f,
            "Power: {} | Accuracy: {} | PP: {}",
            power_str, accuracy_str, self.max_pp
        )?;
