...
impl RewardCalculator {
    ...
    fn find_highest_base_stats(&self, base_stats: &BaseStats) -> Vec<Stat> {
        ...
        let max_value = stats_map.iter().map(|(val, _)| val).max().copied().unwrap_or(0);
    ...
    pub fn moves_learned_at_level(&self, species: Species, level: u8) -> SpeciesDataResult<Vec<crate::Move>> {
        let species_data = crate::get_species_data(species)?;
        
        Ok(species_data.learnset.level_up.get(&level).cloned().unwrap_or_default())
    }
}
...
impl BattleParticipationTracker {
    ...
    pub fn get_participants_against(&self, opponent_player: usize, opponent_pokemon: usize) -> Vec<usize> {
...
mod tests {
    ...
    fn test_experience_groups() {
        ...
        assert_eq!(ExperienceGroup::Fast.calculate_level_from_exp(fast_exp_50 - 1), 49);
        assert_eq!(ExperienceGroup::Fast.calculate_level_from_exp(fast_exp_50), 50);
    }
    ...
    fn test_ev_yield_distribution() {
        ...
        let base_stats = BaseStats {
            hp: 100, attack: 80, defense: 60,
            sp_attack: 40, sp_defense: 40, speed: 30,
        };
    ...
    fn test_evolution_and_move_learning() {
        ...
        match calculator.moves_learned_at_level(Species::Charmander, 7) {
            Ok(moves) => {
                assert!(moves.contains(&crate::Move::Ember), "Charmander should learn Ember at level 7");
            },
